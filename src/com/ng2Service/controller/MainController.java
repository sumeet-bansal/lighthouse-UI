...
public class MainController extends MongoConnector{
	...
	public String fetchList(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if(key.indexOf(',')>-1) {
			...
			if(location.substring(location.indexOf('[')+1, location.indexOf(']')).equals("left")){
				server="left";
	...
	public String csv() {
		...
		if(selL[0].equals("")) {
			...
			cmd("C:/Users/GGupta/Desktop","java -jar ADS_1.0.jar query compare "+path1+" "+path2);
		}
	...
	public void cmd(String jarLoc, String command){
		try {
	        ProcessBuilder builder = new ProcessBuilder("cmd.exe", "/c", "cd \""+jarLoc+"\" && "+command);
